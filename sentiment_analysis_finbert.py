...
if st.session_state.view==1:
    fig_main.add_trace(go.Scatter(
        x=df_fin["date"], y=df_fin["price_change_pct"], name="% Price Change",
        line=dict(color=COLORS["accent"], width=3), connectgaps=False
    ))
    fig_main.add_trace(go.Scatter(
        x=df_fin["date"], y=df_fin["sentiment"], name="Sentiment Score", yaxis="y2",
        line=dict(color=COLORS["gray"], width=2, dash="dot"), connectgaps=False
    ))
    fig_main.update_layout(
        xaxis_title="Date", yaxis=dict(title="% Price Change"),
        yaxis2=dict(title="Sentiment Score", overlaying="y", side="right"),
        template="plotly_dark", height=450, margin=dict(l=10,r=10,t=40,b=20),
        plot_bgcolor=GRAPH_BG, paper_bgcolor=GRAPH_BG
    )
else:
    fig_main.add_trace(go.Scatter(
        x=df_fin["date"], y=df_fin["volume"], name="Trading Volume",
        line=dict(color=COLORS["accent"], width=3), connectgaps=False
    ))
    fig_main.add_trace(go.Scatter(
        x=df_fin["date"], y=df_fin["nb_messages"], name="Messages/Posts", yaxis="y2",
        line=dict(color=COLORS["lightgray"], width=2, dash="dot"), connectgaps=False
    ))
    fig_main.update_layout(
        xaxis_title="Date", yaxis=dict(title="Trading Volume"),
...
chart_title_font = dict(size=16, color="#333333", family="Arial")
...
if st.session_state.mode == "⏳ Slow & more accurate":
    ...
    with col_pie1:
        ...
        fig_pie_sentiment = px.pie(
            ...
            hole=0.4, color_discrete_sequence=[COLORS["accent"], COLORS["gray"], COLORS["inner"]],
            title="Sentiment Distribution"
        )
        fig_pie_sentiment.update_layout(paper_bgcolor=GRAPH_BG, plot_bgcolor=GRAPH_BG,
                                        title_font=chart_title_font, font=chart_font,
                                        height=GRAPH_HEIGHT, title_x=0.5)
...
with col_pie2:
    ...
    fig_pie_sources = px.pie(
        names=list(sources.keys()), values=list(sources.values()),
        hole=0.4, color_discrete_sequence=[COLORS["accent"], COLORS["inner"]],
        title="Source Distribution"
    )
    fig_pie_sources.update_layout(paper_bgcolor=GRAPH_BG, plot_bgcolor=GRAPH_BG,
                                  title_font=chart_title_font, font=chart_font,
                                  height=GRAPH_HEIGHT, title_x=0.5)
